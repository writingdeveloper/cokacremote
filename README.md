...
The `Production Smoke` GitHub Actions workflow also runs hourly (and on manual dispatch) from an external GitHub-hosted runner. It retries both production `/health` endpoints and fails if either endpoint is unavailable, reports a stale tool count/catalog revision, disables OAuth, or diverges in `runtimePolicyFingerprint`. Run the exact same non-destructive check locally with `npm run smoke:production`.
